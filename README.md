...
**Verifiable commerce payments on Kaspa.** Kasway is an open-source
commerce-payment protocol: signed single-use payment intents (KPR-1),
self-custodial wallet authorization, independent chain observation, and
covenant-enforced settlement. Merchants get invoices, checkout, payment links,
and subscriptions; customers never deposit funds into a platform-controlled
balance, and Kasway never touches a private key.
...
1. The API mints a **KPR-1 intent**: a signed JSON document pinning the amount,
   ordered payout split, expiry, and merchant identity (Ed25519 over canonical
   JSON; format in the whitepaper's Appendix A).
2. The customer's wallet fetches the intent from the QR payment-request URI,
   verifies the hash, signature, network, expiry, and outputs — and only then
   signs and broadcasts locally. Every check fails closed.
3. Funds land in a single-use **Kaspa covenant** (P2SH, compiled by
   [SilverScript](https://kasmedia.com/article/hail-the-silverscript)) whose
   spend paths are the only settlement authority.
4. The **chain observer** independently verifies the exact required outputs and
...
Docker Compose stack (server + PostgreSQL):
...
Or against a local PostgreSQL directly:

```bash
cargo test                 # integration + unit tests (needs local PostgreSQL)
...
- `KASPA_NODE_URL` — Kaspa node **wRPC JSON** websocket endpoint (rusty-kaspa's
  stock JSON ports: 18110 mainnet / 18210 TN10 — the JSON listener, not Borsh
  17110/17210; see `crates/kasway-api/src/kaspa_wrpc.rs`).
...
```
crates/
  kasway-db/        # sqlx PostgreSQL pool + embedded migrations
  kasway-covenant/  # SilverScript-compiled Kaspa covenants: escrow_v2 (tiered
                    ...
                    #   covenant script bytes. Apache-2.0.
  kasway-api/       # axum app: lib.rs (build_router), main.rs (bin), handlers,
                    #   auth, background workers, KPR-1. AGPL-3.0.
    tests/          # integration tests: spawn app on ephemeral port, hit it with reqwest
```

`build_router` in `crates/kasway-api/src/lib.rs` is the authoritative list of
endpoints.

## Background workers

Five workers start in-process at boot; no external queue or broker is required:
...
real. Back-office endpoints that were never implemented here were dropped rather
than left as dead schema.

A Tier-3 community-jury dispute layer was prototyped and removed: a bonded jury
rewards agreeing with the majority rather than being right, which degenerates to
voting the base rate without reading the evidence. It is recoverable from git
history if a use case with verifiable evidence appears.

## Contributing

Read the whitepaper first — it defines the trust boundaries the code must keep
(no custody, fail-closed verification, covenant-only settlement). Changes to
